...
import { busPaymentHistory } from "@/api/paymentAPI";
import { BusPaymentData } from "@/types/type";

const Payments = () => {
    const { user } = useAuthStore();
    const [dropdownOpen, setDropdownOpen] = useState(false);
    const [filter, setFilter] = useState<string>("All");
    const [payments, setPayments] = useState<BusPaymentData[]>([]);
    ...
    const fetchPayments = async (selectedFilter: string) => {
        try {
            ...
            const response = await busPaymentHistory(
                Number(userId),
                selectedFilter
            );
                        ...
                        return (
                            ...
                            >
                                ...
                                <View className="flex flex-row justify-center mt-1">
                                    <Text className="text-base font-semibold text-red-950 text-center">
                                        {item.user.name} | {item.user.contactNo}
                                    </Text>
                                </View>
